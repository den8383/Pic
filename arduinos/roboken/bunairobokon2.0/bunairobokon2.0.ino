...
コンパイルには問題ありませんでした。
コントローラーはPS3のやつ使いました。
モタドラは回路で動かせるかまだ試していません。
ただ3回目にマイコンに書き込ませてからなぜか書き込めなくなりました。
BlueTooseを使うためにマイコンにつける奴が調子悪いのでうまくいかなかったら有線でやってみてください。

今のままではおそらくサーボが小鹿になるのでできたらそこも。
急激に逆回転させたときの対策もできたら。
11/02
...
void loop(){
  ...
  if (PS3.PS3Connected){
    ...
    if(PS3.getButtonPress(CIRCLE)){//motor4で持ちあげます
      motor4(-1);
      Serial.print(F("\r\nCircle"));
    }
    ...
    if (PS3.getButtonPress(L1)){//motor1,motor2,motor3,をそれぞれ後転させて左旋回
      motor1(-1);
      motor2(-1);
      motor3(-1);
      Serial.print(F("\r\nL1"));
    ...
    if (PS3.getButtonPress(R1)){//motor1,motor2,motor3,をそれぞれ正転させて右旋回
      motor1(1);
      motor2(1);
      motor3(1);
      Serial.print(F("\r\nR1"));
