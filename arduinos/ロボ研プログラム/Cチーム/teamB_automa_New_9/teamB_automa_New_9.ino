...
motor5つと電磁弁を使用します。
センサーは8つ使用します。
センサーの線は電源の+とー、信号線とLED用の電源用の線です。
プログラムのほうではセンサーが白を読みこんだ時にLEDを点けるというプログラムを書いています。

 init 3秒待機後PHASE_1に移行
 PHASE_1 次の交差点までライントレースします
 PHASE_2 太陽まで進んだ後太陽を取ります。ライントレースする方向が背面側になります。次の交差点までライントレースします。ライントレースする方向が左面側になります
 PHASE_3 ライントレースします。次の交差点までライントレースします。ライントレースする方向が正面側になります。
PHASE_4　次の交差点までライントレースします。P。
PHASE_5　次の交差点までライントレースします。ここで90度左に旋回するために少しだけ左に旋回して
PHASE_6　次の交差を読みこむまで左に旋回します。ここで左90度の旋回を完了します。ライントレースする方向を背面側に変更します。P
PHASE_7 次の交差点までライントレースします。ライントレースる方向を右面側に変更します。
PHASE_8　ライントレースを指定した時間したのち台座を取る動作をします。その後次の交差点までライントレースをします。
PHASE_9 ライントレースを指定した時間したのち大座を取る動作をします。ライントレースする方向を左面側に変更します。次の交差点までライントレースします。
PHASE＿10次の交差点までライントレースします。ライントレースする方向を正面側に変更します。
PHASE_11　次の交差点までライントレースします。ライントレースする方向を左面側に変更します。
PHASE_12　次の交差点までライントレースします。
PHASE_13　次の交差点までライントレースします。ライントレースする方向を背面側に変更します。
PHASE_14 　次の交差点までライントレースします。
PHASE_15　次の交差点までライントレースします。
PHASE_16　少しバックして適当な位置に台座と太陽を置きます。
GOAL  終了
