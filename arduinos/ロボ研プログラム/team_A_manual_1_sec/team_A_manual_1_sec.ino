...
#define PWMPIN1 11//pwm
#define PWMPIN2 10
#define DENJIBENPIN_1 37
#define DENJIBENPIN_2 38
#define pwm_min 125
#define pwm_add 5
//ここまで１
...
void setup() {//ここから
  ...
  pinMode(PWMPIN1,OUTPUT);//PWM用
  pinMode(PWMPIN2,OUTPUT);//PWM用
  //
  pwm_val = 125;
}
void loop() {
  ...
  if (PS3.PS3Connected) {
    ...
    if (PS3.getButtonClick(R2)){
      Serial.print(F("\r\nR2"));
      if(denjiben_count == 1){
        digitalWrite(DENJIBENPIN_1,HIGH);
        digitalWrite(DENJIBENPIN_2,LOW);        
        Serial.print(F("open"));
        denjiben_count--;
      }else if(denjiben_count == 0){
        digitalWrite(DENJIBENPIN_1,LOW);
        digitalWrite(DENJIBENPIN_2,HIGH);        
